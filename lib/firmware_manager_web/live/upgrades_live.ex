defmodule FirmwareManagerWeb.UpgradesLive do
  ...
  def handle_params(params, _uri, socket) do
    ...
    cond do
      fw_file && fw_file != "" and do_action in ["run", "preview"] ->
        ...
        opts = if sys_glob && sys_glob != "", do: Map.put(opts, :sysdescr_glob, sys_glob), else: opts
        ...
        case RuleMatcher.plan_upgrades_multi(selected, opts) do
          {:ok, plan0} ->
            ...
            case do_action do
              "run" ->
                case RuleMatcher.apply_plan_multi(plan, concurrency: 6, poll_ms: 300, poll_attempts: 50) do
                  {:ok, results} ->
                    {:noreply,
                     socket
                     |> assign(:preview, plan)
  ...
  def handle_event("run", _params, socket) do
    case RuleMatcher.apply_plan_multi(socket.assigns.preview, concurrency: 6, poll_ms: 300, poll_attempts: 50) do
      {:ok, results} -> {:noreply, assign(socket, results: results) |> put_flash(:info, "Upgrade plan executing; results updated.")}
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <.header>
        Manual Upgrades
        <:subtitle>Ad-hoc upgrades with preview/dry-run. Validate your criteria here; to persist as a scheduled rule, add it on the Rules page.</:subtitle>
      </.header>

      <div class="bg-gray-800 rounded-lg shadow-md p-6">
        <h3 class="text-md font-semibold text-gray-100 mb-3">Target</h3>
        <p class="text-sm text-gray-300">Rules apply globally across all known CMTS sources. No selection is required.</p>
        <p class="text-xs text-gray-400 mt-2">Tip: After a successful dry-run here, create a persistent scheduled rule on the <a href="/upgrade_rules" class="text-indigo-400 hover:text-indigo-300">Rules</a> page.</p>
      </div>
      ...
      <div class="bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <form phx-submit="preview" class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input name="mac_rule" value={@mac_rule} placeholder="aa:bb:cc:00:00:00/24" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 text-gray-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-300">sysDescr Glob</label>
            <input name="sysdescr_glob" value={@sys_glob} placeholder="%Arris%5.9.3%" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 text-gray-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-300">Firmware File</label>
            <input name="firmware_file" value={@firmware_file} required placeholder="vendor-fw.bin" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 text-gray-100" />
          </div>
          <div>
            <label class="block text-sm text-gray-300">TFTP Server</label>
            <input name="tftp_server" value={@tftp_server} placeholder="10.0.0.5" class="mt-1 block w-full rounded-md border-gray-600 bg-gray-700 text-gray-100" />
          </div>
          <div class="flex items-center gap-3 md:col-span-4">
            <label class="inline-flex items-center text-sm text-gray-300">
              <input type="checkbox" name="force" value="true" checked={@force} class="rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500" />
              <span class="ml-2">Force run (ignore already upgraded)</span>
            </label>
            <div class="grow"></div>
            <button type="submit" name="do" value="preview" class="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-sm">Preview Plan (Dry Run)</button>
            <button type="submit" name="do" value="run" class="px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md text-sm">Run Now</button>
          </div>
        </form>
      ...
      <div class="bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <%= if Enum.any?(@preview) do %>
          ...
          <div class="mt-3 flex justify-end">
            <button phx-click="run" class="px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md text-sm">Run Plan</button>
          </div>
        <% else %>
