defmodule FirmwareManager.Snmp.ModemUpgradeTest do
  ...
  test "modem firmware upgrade flow via SET and polling", %{port: port} do
    ...
    ro = "public"
    # if sim enforces separate communities, adjust accordingly
    rw = "public"
    ...
    assert {:ok, _} =
             SnmpMgr.set(target, file_oid, "cm-fw-1.2.3.bin", community: rw, version: :v2c)
    ...
    status =
      ...
      |> Enum.reduce_while(:unknown, fn _, acc ->
        ...
        case SnmpMgr.get_with_type(target, oper_oid, community: ro, version: :v2c) do
          # accept 4=ok or other success code per sim
          {:ok, {_oid, _type, val}} when val in [4, 5] ->
            {:halt, :ok}
